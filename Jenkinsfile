pipeline {
  ...
  stages {
    ...
    stage('Image build') {
      ...
      steps {
        container('kaniko') {
          sh '/kaniko/executor --context ./ --dockerfile ./dockerfile --destination ${env.HARBOR_URL}/${env.CI_PROJECT_PATH}/${env.APP_NAME}:$BUILD_TAG'
        }
      }
    }
    stage('Deploy') {
      ...
      steps {
        container('helm') {
          sh 'helm upgrade --install --set image.tag=${BUILD_TAG} -n ${env.APP_NAME} --create-namespace ${env.APP_NAME} ./helm-deploy/helm'
        }
      }
    }
  }
